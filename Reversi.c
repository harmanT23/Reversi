...
char oppositeColour(char colour) {
    //Returns opposite colour to given piece
...
bool checkLegalInDirection(char board[26][26], int N, char row, char col,
    ...
    if (positionInBounds(N, row, col) && board[rowInt][colInt] == EMPTY) {
        ...
        if(positionInBounds(N, rowInt + 'a', colInt + 'a') && 
            ...
            //Loops until position not in bound and cell is empty.
...
int scoreAndFlipInDirection(char board[26][26], int dimension, int row, int col, 
    ...
    //Flips tiles in a direction and determines how many flipped
    int positionScore = 0;
    
    //check user wants tiles flipped
    if (doFlip)  
    ...
    //Determines score/flips tiles in given direction
    ...
    return positionScore; //returns score for given position and direction
...
int checkIsValidAndFlip(char board[26][26], int dimension, int row, int col, 
    ...
    //Checks if position valid in at least 1 position
...
int possibleMoves(char board[26][26], int dimension, char colour) {
    ...
    return numMoves; //number of moves for given player
}
...
bool isGameDone(char playerColour, char compColour, char board[26][26], 
        int size) {
    //Determines if no possible available for both players. If true game done. 
...
void whoWon(char board[26][26], int dimension, char playerColour, 
        char compColour) {
    //Determines winner by comparing number of tiles and prints winner.
...
bool computerMoves(char board[26][26], int dimension, char colour) {
    ...
    for(int row = 0; row < dimension; row++) {
        for(int col = 0; col < dimension; col++) {
            ...
            if(biggestScore < checkIsValidAndFlip(board, dimension, row, col, 
                    colour, false)) {
...
void gameTurns(char board[26][26], int dimension) {
    ...
    else //computer wins if invalid value entered
        printf("%c player wins.", compColour); 
...
int main(void) {
    ...
    scanf("%d", &size);
    char reversiBoard[size][size]; //declares board size. 
